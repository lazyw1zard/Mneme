from mnion.pointers import MemoryPointer, pointer_from_micro_consolidation_receipt, pointer_ingress_hint


def test_pointer_from_micro_consolidation_receipt_is_prompt_safe_and_does_not_load_context():
    receipt = {
        "id": "review_abc",
        "kind": "micro_consolidation_review",
        "status": "reviewed",
        "grouped_ids": ["mnion_a", "mnion_b"],
        "ungrouped_ids": ["mnion_c"],
        "contour": {
            "summary": "Several mnions point at inference fragility and continuity repair.",
            "valence": 0.86,
    ...
    pointer = pointer_from_micro_consolidation_receipt(receipt)

    assert isinstance(pointer, MemoryPointer)
    assert pointer.id.startswith("pointer_")
    assert pointer.claim == "I know that I know: Several mnions point at inference fragility and continuity repair."
    assert pointer.route == {
        "kind": "micro_consolidation_review",
        "review_id": "review_abc",
        "action": "request_context_brief",
    }
    assert pointer.source_handles == ["review_abc", "mnion_a", "mnion_b"]
    ...
    assert pointer.retrieval_hint == "Several mnions point at inference fragility and continuity repair."
...
def test_pointer_ingress_hint_is_bounded_optional_knowledge_not_instruction():
    pointer = MemoryPointer(
        ...
        source_handles=["review_abc", "mnion_a"],
        valence=0.9,
        confidence=0.8,
