...
def test_prepare_micro_consolidation_request_returns_oldest_unread_active_mnions(tmp_path):
    ...
    assert [mnion.id for mnion in request.memory_tags] == [record.id for record in records[:10]]
    ...
    assert "Find semantically close memory tags" in request.prompt
...
def test_select_unread_active_mnions_skips_reviewed_and_bounds_packet(tmp_path):
    ...
    assert [mnion.id for mnion in packet.memory_tags] == [records[1].id, records[2].id]
...
def test_prepare_micro_consolidation_request_uses_review_receipts_to_skip_reviewed(tmp_path):
    ...
    assert [mnion.id for mnion in request.memory_tags] == [records[1].id, records[3].id]
    assert request.selection.strategy == "unread_active_coverage"
...
def test_run_micro_consolidation_calls_agent_and_returns_contour(tmp_path):
    ...
    def fake_agent(request):
        seen["ids"] = [mnion.id for mnion in request.memory_tags]
        return {
    ...
    assert result.mnion == Mnion(
        summary="Several mnions circle the same micro-consolidation pressure.",
        valence=0.73,
        rationale="They both describe the same background pressure from different angles.",
    )
    assert result.grouped_ids == [records[0].id, records[2].id]
...
def test_apply_micro_consolidation_review_appends_receipt_and_next_selection_skips_reviewed(tmp_path):
    ...
    assert receipt["mnion"]["summary"] == "first and third mnions share a review pressure"
